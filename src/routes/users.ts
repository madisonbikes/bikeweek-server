    ...
    */
      ...
      .get(
        ...
        asyncWrapper(async (request, response) => {
          ...
          const dbUser = await userModel.findUserById(
      ...
      .put(
        ...
        asyncWrapper(async (request, response) => {
          ...
          const federatedId =
            await googleFederatedVerifier.verifyFederatedToken(
              data.validateToken,
            );
      ...
      .delete(
        ...
        asyncWrapper(async (request, response) => {
          const authUser = request.user as AuthenticatedUser;
          const dbId = new ObjectId(authUser.id);
          const provider = federatedProviderSchema.parse(
            request.params.provider,
          );
          const result = await userModel.disconnectFederatedProvider(
            dbId,
            provider,
          );
          if (!result) {
